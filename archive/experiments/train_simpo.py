"""
Simple Preference Optimization (SimPO) Training Script

SimPO is a simpler variant of DPO that doesn't require a reference model.
More memory-efficient and often achieves comparable results.
...
Usage:
    python train_simpo.py --model gemma --max_samples 1000
    python train_simpo.py --model llama --sft_checkpoint ./outputs/sft/llama/final
"""
...
def setup_logging(model_name: str, output_dir: str) -> logging.Logger:
    ...
    log_file = log_dir / f"simpo_{model_name}_{timestamp}.log"

    logger = logging.getLogger(f"simpo_{model_name}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="SimPO Training for Mental Health SLMs")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="SimPO gamma (target reward margin)",
    )
    parser.add_argument("--cpo_alpha", type=float, default=1.0, help="CPO alpha weight")
    ...
    # Output
    parser.add_argument("--output_dir", type=str, default="./outputs/simpo")
    parser.add_argument("--use_wandb", action="store_true")
    parser.add_argument("--wandb_project", type=str, default="refine-lab-simpo")
...
def prepare_preference_dataset(dataset_name: str, max_samples: int, logger):
    """Prepare preference dataset for SimPO."""
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("SimPO Training Started")
    logger.info("=" * 60)
    logger.info(f"Model: {model_id}")
    logger.info(f"SFT Checkpoint: {args.sft_checkpoint or 'None (training from base)'}")
    logger.info(f"Dataset: {args.dataset}")
    logger.info(f"Max samples: {args.max_samples}")
    logger.info(f"SimPO Gamma: {args.simpo_gamma}")
    logger.info(f"CPO Alpha: {args.cpo_alpha}")
    ...
    if args.use_wandb:
        ...
        wandb.init(
            ...
            name=f"simpo_{args.model}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config=vars(args),
        )
    ...
    training_args = CPOConfig(
        ...
        seed=args.seed,
        # SimPO specific - use simpo loss type
        loss_type="simpo",
        simpo_gamma=args.simpo_gamma,
        cpo_alpha=args.cpo_alpha,
        max_length=args.max_length,
    ...
    logger.info("Initializing CPO Trainer with SimPO loss...")
    trainer = CPOTrainer(
        model=model,
        args=training_args,
